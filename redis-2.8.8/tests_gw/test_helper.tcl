...
source tests_gw/support/redis.tcl
source tests_gw/support/server.tcl
source tests_gw/support/tmpfile.tcl
source tests_gw/support/test.tcl
source tests_gw/support/util.tcl
...
##### SSS Begin #####
set ::with_smr 1; # If "1" this means, we are running redis with smr log replicator
set ::smr_replicator "smr-replicator"
set ::with_gw 0; # if "1" this means, we are running redis with gateway
set ::gwport 6379
##### SSS End #######
...
proc redis_deferring_client {args} {
    ...
    set client [redis $::host $::gwport 1]
...
proc cleanup {} {
    ...
    flush stdout
    catch {exec rm -rf {*}[glob tests_gw/tmp/redis.conf.*]}
    catch {exec rm -rf {*}[glob tests_gw/tmp/server.*]}
    if {!$::quiet} {puts "OK"}
}
