...
proc assert_match {pattern value} {
    puts "assert_match Expected '$value' to be equal to '$pattern'"
...
# [MODIFIED NBASE_ARC] for gateway test
#    set dbg [r debug object $key]
#    while {[string match "* swapped at:*" $dbg]} {
#        r debug swapin $key
#        set dbg [r debug object $key]
#    }
    set dbg [r object encoding $key]
    assert_match "$enc" $dbg
}
...
proc test {name code {okpattern undefined}} {
    ...
    if {[catch {set retval [uplevel 1 $code]} error]} {
        ...
        puts "assert_match Expected '$okpattern' to be equal or match '$retval'"
