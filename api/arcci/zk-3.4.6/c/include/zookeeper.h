...
#ifdef _WIN32
/**
 * This function-pointer will be indicated a method that connects to server and attachs to iocp by arcci.
 */
typedef int(*sock_nonblock_func)(char *err, char *addr, int port);
/**
 * This function-pointer will be indicated a method that closes socket and detaches from iocp by arcci.
 */
typedef int (*sock_close_func)(SOCKET);
#endif
...
#else
ZOOAPI zhandle_t *zookeeper_init(const char *host, watcher_fn watcher,
  int recv_timeout, const clientid_t *clientid, void *context, int flags,
