...
public class LeaderElectionSupport implements Watcher {
    ...
    private State state;
    private final Set<LeaderElectionAware> listeners;
    ...
    public synchronized void start() {
        state = State.START;
        dispatchEvent(LeaderElectionEventType.START);
    ...
    private void makeOffer() throws KeeperException, InterruptedException {
        state = State.OFFER;
        dispatchEvent(LeaderElectionEventType.OFFER_START);
        ...
        try {
            leaderOffer.setNodePath(zooKeeper.create(rootNodeName + "/" + "n_",
                    hostName.getBytes(config.getCharset()), ZooDefs.Ids.OPEN_ACL_UNSAFE,
        ...
        dispatchEvent(LeaderElectionEventType.OFFER_COMPLETE);
    }
    ...
    private void determineElectionStatus() throws KeeperException,
        ...
        dispatchEvent(LeaderElectionEventType.DETERMINE_START);
        ...
        for (int i = 0; i < leaderOffers.size(); i++) {
            ...
            if (leaderOffer.getId().equals(this.leaderOffer.getId())) {
                ...
                dispatchEvent(LeaderElectionEventType.DETERMINE_COMPLETE);
    ...
    private void becomeReady(LeaderOffer neighborLeaderOffer)
        ...
        dispatchEvent(LeaderElectionEventType.READY_START);
        ...
        if (stat != null) {
            ...
            state = State.READY;
            dispatchEvent(LeaderElectionEventType.READY_COMPLETE);
        } else {
            /*
    ...
    private void becomeLeader() {
        state = State.ELECTED;
        dispatchEvent(LeaderElectionEventType.ELECTED_START);

        logger.info("Becoming leader with node:{}", leaderOffer.getNodePath());

        dispatchEvent(LeaderElectionEventType.ELECTED_COMPLETE);
    }
    ...
    private void becomeFailed(Exception e) {
        ...
        state = State.FAILED;
        dispatchEvent(LeaderElectionEventType.FAILED);
    }
     ...
     * offers from ZooKeeper to determine who the leader is (i.e. there is no
    ...
    private List<LeaderOffer> toLeaderOffers(List<String> strings)
        ...
        for (String offer : strings) {
            String hostName;
            try {
                hostName = new String(zooKeeper.getData(rootNodeName + "/"
                        + offer, false, null), config.getCharset());
            } catch (UnsupportedEncodingException e) {
    ...
    public void process(WatchedEvent event) {
        if (event.getType().equals(Watcher.Event.EventType.NodeDeleted)
                && !event.getPath().equals(leaderOffer.getNodePath())
                && state != State.STOP) {
            logger.debug(
    ...
    private void dispatchEvent(LeaderElectionEventType eventType) {
    ...
    public String toString() {
        return "{ state:" + state + " leaderOffer:" + leaderOffer
                + " zooKeeper:" + zooKeeper + " hostName:" + hostName
                + " listeners:" + listeners + " }";
    }
     ...
     * The type of event.
     */
    public static enum LeaderElectionEventType {
